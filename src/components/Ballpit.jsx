...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    // Make sure the canvas has explicit pixel dimensions before initializing Three
    try {
      const rect = canvas.getBoundingClientRect();
      const w = Math.max(1, Math.floor(rect.width));
      const h = Math.max(1, Math.floor(rect.height));
      // set both DOM attributes and style so ThreeRenderer picks up correct size immediately
      canvas.width = w;
      canvas.height = h;
    ...
    console.debug('[Ballpit] initializing (canvas size)', canvas.width, canvas.height);
    spheresInstanceRef.current = createBallpit(canvas, { followCursor, ...props });

    // give layout a tick then force a resize in the Three instance in case parent sizes changed
    setTimeout(() => {
      try {
  ...
  // Position canvas absolutely so it fills the wrapper and particles are visible above background
  return <canvas className={className} ref={canvasRef} style={{ position: 'absolute', left: 0, top: 0, width: '100%', height: '100%' }} />;
};
